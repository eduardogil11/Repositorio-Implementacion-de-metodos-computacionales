...
//Librería especial para paralelización del programa
#include <omp.h>
...
int main(int argc, char* argv[]) {
	...
	//Variables para guardar la suma de los números primos y los tiempos de ejecucion
	long long suma = 0;
	unsigned t0, t1;
  ...
  //Guarda el tiempo en que se inicio el programa
	...
	//Define la región paralela
  #pragma omp parallel
  {
    //Le dice a la parelización que trabajaremos con for y con la variable "suma" que se le estará sumando
    ...
    for(long long i = 0; i < 5000000; i++) {
      ...
      if (verificar_primo(i)) {
        //Le dice a la paralelización que haga las sumas con precaución
        #pragma omp critical 
        suma += i;
      }
    }
